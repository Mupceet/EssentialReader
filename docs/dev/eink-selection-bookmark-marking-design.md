...
9. **触发带**：页顶/页底按住触发翻页的端点吸附与跟手性；正常拖拽调界、行盒
   间空档、空白区长按拖拽死手势不得误触发。
10. **页面书签**：顶栏钮选中态（随快照 bookmarkBadge）、下拉 toggle、页角标随
   重排刷新、同页多条时删最近一条；跨模式互认（eink 书签/划线/想法 ↔ 宿主
   显示，宿主建的波浪/双线在 eink 如实显示）。
11. **想法弹层软键盘**：IME 避让不压缩阅读视口，正文不重排不跳页。
12. **降级宿主**：注释 `EInkBridge` 的 selectionEngine 装配 → 长按选择整体不启用
   （不选词、松手无动作）、下拉与顶栏书签钮不渲染，无假死路径。
13. **含标题选区**：静默不落划线（含标题的跨页会话同样不落），操作条只留复制键。
14. **onLayoutException 恢复路径**。
