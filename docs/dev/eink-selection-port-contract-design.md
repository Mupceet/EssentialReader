...
- 破坏性契约变更：**0.7.1（待发布）**。原计划并入 0.7.0「契约清理轮」，但实施期
  发现 0.7.0 已在 `origin/eink/lib` 定版发布（`0caf7a80e`，2026-09-20，含字节端口/
  死成员清理/依赖保守档），本地子模块指针（30dfb59c3）落在定版之前——本地工作
  分支与远端已分叉，推送时的 rebase/merge 策略由维护者决定。本轮在
  `modules/eink/build.gradle.kts` 追加 `0.7.1` 版本注释块（对齐「待发布」先例：
  注释先行、版本号发布时统一翻）。
