...
- **Layered feedback language (spec §35/§42, mandatory)**: pressed = instant inversion (the only pressed language), with a 120ms minimum hold, provided by `rememberImmediatePressState` + `eInkActionColors` in `:modules:eink` — never hand-written per-screen inversion color logic. Persistent selection: small controls use solid fill (primary/onPrimary); operation-bar icon buttons (unified `EInkOperationBarIcon`, used by the bottom operation bar and the reader's top/bottom bars) swap to a paired filled icon variant (`_e` outline → `_s` filled, View-version bottom-bar naming — assets must be provided in pairs, including future ones) on a white container, falling back to solid fill (primary/onPrimary) when no paired asset exists; long-list rows use a left solid marker (▮) + bold title, never full-row inversion. Disabled = dedicated mid-gray, never alpha. Border emphasis is reserved for focus. New or updated interactive components must comply (see spec §74 checklist items 11/12 and §78 Rule 11).
