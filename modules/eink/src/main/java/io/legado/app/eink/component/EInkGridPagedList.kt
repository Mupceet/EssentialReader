 ...
 */
@Stable
class EInkGridPagedListState(val gridState: LazyGridState) : EInkPageController {
    ...
    /**
     * 从保存的状态恢复分页（导航返回，经 [rememberEInkGridPagedListState]
     ...
     * [EInkPagedListState.realignToPageStart]）：网格重排后首可见项与
...
/**
 * 创建并记住 [EInkGridPagedListState]，内部在首次布局时自动测量页项数。
 *
 * 分页状态经 [rememberSaveable] 随导航栈条目保存恢复，与
 * [rememberLazyGridState] 的滚动位置恢复保持一致。
 */
@Composable
fun rememberEInkGridPagedListState(): EInkGridPagedListState {
    val gridState = rememberLazyGridState()
    val saver = remember(gridState) {
        Saver<EInkGridPagedListState, List<Any>>(
            save = { listOf(it.pageStart, it.pageItemCount) },
            restore = { values ->
                EInkGridPagedListState(gridState).apply {
                    restorePaging(values[0] as Int, values[1] as Int)
                }
            }
        )
    }
    val state = rememberSaveable(saver = saver) { EInkGridPagedListState(gridState) }
