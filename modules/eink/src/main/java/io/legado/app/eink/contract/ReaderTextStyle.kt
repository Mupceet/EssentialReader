 ...
 * 阅读排版参数快照（渲染 + 设置面板双用途）。
 *
 * 模块对「阅读页长什么样」的全部主张收敛在这一个值对象：阅读页设置面板
 * 编辑它，经 [ReaderEngine.applyStyle] 整体写入宿主排版引擎（映射宿主
 * 阅读配置各字段并持久化 + 刷新画笔）；[ReaderEngine.currentStyle] 从
 * 宿主配置读回同构快照。这些键与宿主完整模式的阅读排版设置共享存储——
 * E-Ink 调参会同步影响完整模式，反之亦然。
 *
 * 字段单位与模块内编辑区间（设置面板滑条的 coerce 边界，常量在
 * feature/reader 包内）：
 *  - [textSize] sp，8..40。标题字号无独立字段——本模块钉平标题 = 正文，
 *    宿主实现把标题字号一并写入同值（develop/E-Ink 语义，防止完整模式
 *    残留的绝对标题字号跟随失效）；
 *  - [letterSpacing] em（引擎侧乘以字号换算 px），0..0.5，面板按 0.05
 *    步进设置，避免浮点累加漂移；
 *  - [indentChars] 段首缩进字符数，0..4，<=0 即无缩进；宿主把它展开为
 *    缩进字符串写入（展开所用缩进字符是宿主引擎常量，不进快照）；
 ...
 *    区间同正文边距。
 *
 * 区间是模块 UI 的编辑边界而非宿主校验义务：模块侧调参前已 coerce，
 * 宿主实现按自身引擎容忍度原值透传即可（对齐 View 版行为）。
 */
