...
interface ReaderEngine {
     ...
     *
     * 静默规格（本仓参照实现口径，宿主门槛可自选、语义须一致）：
     *  - fire-and-forget：无返回值、无进度/结果回调。检查中、被限频跳过、
     *    无更新、检查失败四种结局都不产生任何界面反馈；
     *  - 限频窗口内重复调用直接跳过（本仓 10 分钟，锚定书籍记录的
     *    lastCheckTime）——阅读页每次进入都自动触发本方法，短时间内重进
     *      的第二次起是**被跳过**而不是「检查过没更新」；
     *  - 仅真正拉到新增章节才入库并推送（章数不变时不推）；
     *  - 检查挂在宿主进程级作用域，不随阅读页退出取消；完成时阅读回调
     *    已注销则结果丢弃（对齐 [ReaderEngineCallback] 无注册者语义），
     *    数据已入库，此后进入目录页自然取到新目录。
     *
     * 「检查中 / 已是最新」的用户可见反馈需要宿主另行提供状态通道，契约
     * 暂无——模块不得在触发处自造「正在检查更新」提示（限频跳过时即
     * 成为误导）。
     */
    fun refreshToc()
