...
上表后四行为可选端口：不注册即降级，UI 以入口消失明示缺席，不以空实现
...
- **能力裁剪方言总表**：宿主「不具备某能力」在契约面有五种表达，每种
  ...
  |---|---|---|
  | 可选端口不注册 | app 更新 / 书签笔记（`MarksEngine`，阅读内与目录页两表面一起降级） / 书架分组 | 对应入口整体不渲染 |
  | 端口能力声明 = false | 段评气泡（`supportsReviewBubbles`）；书签/笔记两特性粒度（`supportsMarkings`/`supportsBookmarks`，各管阅读内+目录页两表面） | 仅隐藏该能力入口，端口其余功能保留 |
