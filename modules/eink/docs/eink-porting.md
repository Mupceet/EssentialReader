...
| ReaderEngineImpl | **排版写入必须走 ReadStyleGateway**（`:verifyConfigArchitecture` 护栏拦 `ReadBookConfig.* =` 直写，且正则无 `(?!=)` 排除——`ReadBookConfig.textBold == 1` 这种**比较**也会误中，用 `.let { it == 1 }` 规避）：17 个 `ReadStyleMutation`（TextSize/LetterSpacing/ParagraphIndent/LineSpacing/ParagraphSpacing/Padding*/Header/FooterPadding*/TextBold 全有键）逐个 `updateCurrentStyle` 后显式 `save()`（updateCurrentStyle 只改内存+publishState）；**不写 TitleSize**——E-Ink 渲染层标题 = 正文字号 + 加粗 + 正文体，宿主标题字号设置被忽略（行盒仍按宿主 titleSize 排布）；随后 `ChapterProvider.upStyle()`；另实现 `contentTextTypeface` 端口（返回 `ChapterProvider.contentPaint.typeface`，与排版测量同源，模块画布据此渲染） |
| ReaderPageSnapshotMapper | **纯几何映射**：快照不携带渲染参数（ReaderPaintSpec/ReaderShadowSpec 已废——模块画布按自身 ReaderTextStyle + `contentTextTypeface` 端口渲染，API35+ 字距半格补偿由画布绘制期自算）；`ImageColumn` 自带 `book` 字段——映射闭包捕获 `column.book`（换书瞬间旧页不误取新书），尺寸 ≤ 0 与解码异常均返回 null；图片抗锯齿不经参数——`GlobalSettings.useAntiAlias` 由模块画布自取，宿主实现从 OtherSettingsGateway 读 |
